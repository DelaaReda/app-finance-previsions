...
- Timestamp (UTC): `2026-03-10T14:16:18Z`
...
- `bash scripts/fc_doctor.sh --json`
  - `rc=1 status=degraded`
  - scoped checks: `sessions=ok queue_workboard=ok providers=ok product_value=ok delivery_integrity=ok`
  - residual noise: `capability_result_integrity=degraded`
...
- No Strategy Playbooks runtime or infra defect reproduced in the VM workspace.
- Residual degraded noise is shared observability debt, not a BATCH-15-specific outage:
  - `health_snapshot.sh` still marks planner staleness.
  - `fc_doctor.sh --json` is top-level `degraded` only because `capability_result_integrity` is degraded while task-scoped runtime checks stay green.

...
- Timestamp (UTC): `2026-03-10T13:22:52Z`
...
- `fc_status_brief.sh`: PASS, `Santé: OK`, `Blocages: none`, `mismatch_count=0`.
- `health_snapshot.sh`: mixed, returned `health=STALE` with `blocked=[]`, `stale=[]`, `critical_widgets=stale`.
- `monitor_contract_smoke.sh`: PASS, `health=OK roles=1 agents=4 queue_states=4 workboard_ready=0`.
- `stale_cron_sweep.sh --dry-run`: PASS, `matched=0 stale=0 reset_failed=0`.
- Strategy playbooks route test slice: PASS, `6 passed`.
- `fc_doctor.sh --json`: top-level `status=degraded`, but scoped checks for `runtime_state`, `scheduler_authority`, `sessions`, `locks`, `queue_workboard`, `providers`, and `product_value` were all `ok`.

...
Exemple:
`task_update=complete;lock_check=ok;run_note=Ajoute un cache stable et tests;exec_report=cache_backend_ajoute_et_tests_lances;issues=none;suggestions=none;arch_rule=api_contract;review_scope=BATCH-02-BE-123;conformance=PASS;violations=none;stream_id=BATCH-02;task_id=BE-123;backend_artifact=apps/api/src/platform/main.py;cmd=bash scripts/backend_regression_gate.sh --no-live;tests_run=pytest:PASS`
...
- `intent_registry_ref=<registry#intent_id>`
- `edit_scope=<zone_touchee>`
- `reflection_passes=<nombre>=5` (minimum 5 passes de réflexion avant changement)
...
- `intent_registry_ref=<registry#intent_id>` (trace canonique du claim d intention)
...
- `claim`:
  - `stream_id`, `task_id`
  - `intent_id`, `intent_chat_ref`, `intent_memory_ref`, `intent_registry_ref`, `edit_scope`
...
## Contrainte architecture (obligatoire)
- `conformance=BLOCKED` implique un contrat role `STATUS: BLOCKED`, `VERDICT: BLOCKED`, et `BLOCKER_ID != NONE`.
- Si `violations=none`, `conformance` ne doit pas etre `BLOCKED`.

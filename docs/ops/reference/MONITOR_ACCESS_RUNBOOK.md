...
curl -sS http://127.0.0.1:7779/api/status?lite=1 | jq '{primary_status,product_runtime,execution_mode,core_roles}'
curl -sS http://192.168.64.9:7780/api/status?lite=1 | jq '{primary_status,product_runtime,execution_mode,core_roles}'
```
...
Use `scripts/monitor_stack_guard.sh` only to maintain monitor/LAN proxy reachability. It is not the canonical launcher for the full product stack.

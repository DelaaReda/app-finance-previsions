...
- App publique = AWS EC2, pas la VM UTM.
- Validation produit: `http://3.98.20.77/`, `http://3.98.20.77/api/...`, `http://3.98.20.77:8080/...`
- Orchestration/control-plane: VM UTM seulement.
- `localhost:8050`, `localhost:5173`, `localhost:7779` ne sont valides que pour un debug local explicitement intentionnel sur l’hôte concerné, jamais comme preuve produit d’équipe.
...
# Redémarrer l'app AWS:
bash scripts/aws_remote_app_control.sh restart
```

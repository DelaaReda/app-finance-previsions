...
def _configured_econ_mode() -> str:
    explicit = os.getenv("ECON_AGENT_MODE", "").lower().strip()
    if explicit:
        return explicit
    llm_mode = os.getenv("LLM_MODEL_MODE", "").lower().strip()
    if llm_mode in {"dev", "fast", "test", "testing"}:
        return "dev"
    if get_llm_settings is not None:
        try:
            s = get_llm_settings()
            mode = (getattr(s, "econ_agent_mode", "") or "").lower().strip()
            if mode:
                return mode
            llm_mode_default = (getattr(s, "llm_mode_default", "") or "").lower().strip()
            if llm_mode_default in {"dev", "fast", "test", "testing"}:
                return "dev"
        except Exception:
            pass
    return "prod"
...
DEV_MODELS = (
    [m.strip() for m in os.getenv("LLM_DEV_MODELS", "").split(",") if m.strip()]
    or [
...
class EconomicAnalyst:
    ...
    def __init__(
        ...
        if MODE == "dev":
            ...
            # Hint g4f provider preference for fast path
            if get_llm_settings is not None:
                os.environ.setdefault("G4F_PROVIDER", get_llm_settings().g4f_provider)
            base = DEV_MODELS[:]
            ...
            base = env_models or model_candidates or ranked_tested_models or DEFAULT_MODEL_CANDIDATES

...
WORKING_PATH = Path(__file__).resolve().parents[2] / "data" / "llm" / "models" / "working.json"
...
def _provider_for_model(model: str) -> Optional[str]:
    ...
    We only force paid providers when their API key is present.
    Otherwise we keep provider=None so g4f can use auto/no-auth paths.
    """
    m = (model or "").lower()
    has_openrouter = bool(os.getenv("OPEN_ROUTER_API_KEY"))
    has_deepinfra = bool(os.getenv("DEEPINFRA_API_KEY"))
    if "deepseek" in m:
        return "deep-infra" if has_deepinfra else None
    if "qwen" in m:
        return "deep-infra" if has_deepinfra else None
    if "phi-4" in m:
...
def _static_candidates() -> List[str]:
    # fallback: use static power list from econ_llm_agent, then a hardcoded safety list
    try:
        from analytics.econ_llm_agent import POWER_NOAUTH_MODELS
        models = [m for m in (POWER_NOAUTH_MODELS or []) if m]
        if models:
            return models[:]
    except Exception:
        pass

...
try:
    from services.service_standard import safe_float, safe_int, utc_now_iso
except Exception:  # pragma: no cover
    from src.services.service_standard import safe_float, safe_int, utc_now_iso  # type: ignore
...
def _point_key(point: Dict[str, Any]) -> Tuple[str, int, int]:
    return (
        ...
        safe_int(point.get("horizon_days"), 0),
        safe_int(point.get("min_samples"), 0),
    )


def build_tracking_point(report: Dict[str, Any]) -> Dict[str, Any]:
    as_of_value = str(report.get("as_of") or utc_now_iso())
    ...
    return {
        "as_of": as_of,
        "date": as_of[:10],
        "horizon_days": safe_int(report.get("horizon_days"), 5),
        "min_samples": safe_int(report.get("min_samples"), 20),
        "evaluated_rows": safe_int(coverage.get("evaluated_rows"), 0),
        "total_rows": safe_int(coverage.get("total_rows"), 0),
        "with_price_series": safe_int(coverage.get("with_price_series"), 0),
        "edge_vs_baseline": safe_float(overall.get("edge_vs_baseline")),
        "hit_rate": safe_float(overall.get("hit_rate")),
        "brier": safe_float(overall.get("brier")),
        "calibration_error": safe_float(overall.get("calibration_error")),
...
def build_tracking_payload(
    *,
    ...
    scope_points = [
        p
        for p in points
        if safe_int(p.get("horizon_days"), -1) == point["horizon_days"]
        and safe_int(p.get("min_samples"), -1) == point["min_samples"]
    ]
    ...
    return {
        ...
        "kpis": {
            ...
            "avg_evaluated_rows_last_7": _average(
                [safe_float(item.get("evaluated_rows")) for item in trailing_scope]
            ),
            "avg_edge_vs_baseline_last_7": _average(
                [safe_float(item.get("edge_vs_baseline")) for item in trailing_scope]
            ),
        },
        "points": points,
    }

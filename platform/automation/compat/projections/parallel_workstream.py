...
AUTOMATION_DIR = THIS_DIR.parents[1]
if str(AUTOMATION_DIR) not in sys.path:
    sys.path.insert(0, str(AUTOMATION_DIR))

from planning.plane.plane_planning import build_plane_planning_snapshot
from orchestrator_paths import canonical_docs_root, resolve_orchestrator_write_path, write_orchestrator_json

DEFAULT_BOARD = Path("logs-codex-runs/orchestrator-state/parallel-workstreams.json")
DEFAULT_PRIORITY_QUEUE = Path("logs-codex-runs/orchestrator-state/priority-queue.json")
...
DEFAULT_STEP_NOTES: Dict[str, List[str]] = {
    "PLAN": [
        "GOVERNANCE-NOTE: keep queue/workboard in sync; run platform/automation/runtime/planner/planner_runtime_actions.py sync-priority when drift is detected.",
    ],
    "DEV-01": [
...
def save_board(path: Path, board: dict) -> None:
    board["projection"] = True
    board["projection_kind"] = "parallel_workstreams"
    board["projection_source"] = "parallel_workstream"
...
def default_board() -> dict:
    return {
        "version": 1,
        "projection": True,
        "projection_kind": "parallel_workstreams",
        "projection_source": "parallel_workstream",
        "canonical_truth": "sqlite_event_store",
...
def _stamp_priority_queue_projection(queue_obj: dict) -> None:
    meta = queue_obj.get("meta")
    if not isinstance(meta, dict):
        meta = {}
        queue_obj["meta"] = meta
    meta["projection"] = True
    meta["projection_kind"] = "priority_queue"
    meta["projection_source"] = "parallel_workstream"
    meta["canonical_truth"] = "sqlite_event_store"
    meta["planner_scheduler"] = "planner"


def _is_canonical_projection_target(path: Path, relative_name: str) -> bool:
    target = Path(path)
    runtime_target = resolve_orchestrator_write_path(ROOT, relative_name, create_parent=False)
    docs_target = canonical_docs_root(ROOT) / relative_name
    return target == runtime_target or target == docs_target


def _write_projection_json(path: Path, relative_name: str, payload: dict) -> None:
    if _is_canonical_projection_target(path, relative_name):
        write_orchestrator_json(ROOT, relative_name, payload)
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")


def _plane_front_door_active(workspace_root: Path) -> bool:
    try:
        snapshot = build_plane_planning_snapshot(workspace_root)
    except Exception:
        return False
    ...
    sync_active = bool(sync.get("adapter_enabled")) or bool(cache.get("exists"))
    return sync_active and snapshot.get("new_backlog_creation_allowed_in_docs") is False
...
def reconcile_state(board: dict, queue_path: Path) -> Dict[str, int]:
    ...
    def _normalize_active_cycle_payload(payload: dict) -> bool:
        ...
        closed_states = {"DONE", "CLOSED", "CANCELLED", "ARCHIVED", "PASS"}
        desired_active: list[str] = []
        seen_active: set[str] = set()
        for stream_id, state in stream_states_from_wb.items():
            if not stream_id or state in closed_states or stream_id in seen_active:
                continue
            desired_active.append(stream_id)
            seen_active.add(stream_id)

        migrated_recent = [item for item in current_active if item not in seen_active]
        desired_recent: list[str] = []
        seen_recent: set[str] = set()
        for item in migrated_recent + current_recent:
            if not item or item in seen_active or item in seen_recent:
                continue
            desired_recent.append(item)
            seen_recent.add(item)

        changed = desired_active != current_active or desired_recent != current_recent
        if changed:
            active_cycle["active_batch_ids"] = desired_active
            active_cycle["recent_completed_batch_ids"] = desired_recent[:5]
        return changed
...
def planner_autobatch(
    ...
    *,
    ...
    if _plane_front_door_active(workspace_root):
        append_event(
            board,
            "planner_autobatch_skipped",
            {
                "reason": "plane_front_door_active",
...
def claim_task(
    ...
    if _active_count > _wip_limit:
        raise RuntimeError(
            ...
            f"Inspect the runtime workboard and reduce active WIP before retrying."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compat projections and legacy workboard mutations; non-canonical runtime surface")

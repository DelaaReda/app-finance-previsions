...
from runtime.model_plane import resolve_secondary_codex_fallback as model_plane_resolve_secondary_codex_fallback
...
COMPAT_ALLOWED_WORKER_TYPES = {"qa_review_worker"}
_OPENCLAW_CONTROL_PLANE_PATH = Path(__file__).resolve().parents[2] / "operator" / "openclaw" / "openclaw_control_plane.py"
_OPENCLAW_CONTROL_PLANE_SPEC = spec_from_file_location(
    "fc_operator_openclaw_control_plane_worker_manager",
    _OPENCLAW_CONTROL_PLANE_PATH,
)
if _OPENCLAW_CONTROL_PLANE_SPEC is None or _OPENCLAW_CONTROL_PLANE_SPEC.loader is None:
    raise ImportError(f"unable to load canonical openclaw control plane from {_OPENCLAW_CONTROL_PLANE_PATH}")
_OPENCLAW_CONTROL_PLANE_MODULE = module_from_spec(_OPENCLAW_CONTROL_PLANE_SPEC)
_OPENCLAW_CONTROL_PLANE_SPEC.loader.exec_module(_OPENCLAW_CONTROL_PLANE_MODULE)
sync_canonical_skills = _OPENCLAW_CONTROL_PLANE_MODULE.sync_canonical_skills
...
def _load_config(root: Path) -> WorkerManagerConfig:
    ...
    state_dir_raw = str(os.environ.get("FC_DYNAMIC_WORKERS_STATE_DIR", "") or "").strip()
    if state_dir_raw:
        orch_dir = Path(state_dir_raw).expanduser()
        if not orch_dir.is_absolute():
            orch_dir = (root / orch_dir).resolve()
    else:
        orch_dir = resolve_orchestrator_write_path(root, "dynamic-workers-registry.json").parent.resolve()
...
def _secondary_codex_model(current_model: Any) -> tuple[str, str]:
    model, thinking = model_plane_resolve_secondary_codex_fallback(
        "worker",
        current_model,
        SECONDARY_CODEX_DEFAULT_THINKING,
        env=os.environ,
    ...
    return token, str(thinking or SECONDARY_CODEX_DEFAULT_THINKING).strip() or SECONDARY_CODEX_DEFAULT_THINKING


def _worker_orphan_ids(records: list[WorkerRecord]) -> list[str]:
    if not shutil_which("openclaw"):
        return []
    tracked = {str(record.worker_id or "").strip() for record in records if str(record.worker_id or "").strip()}
    orphans: list[str] = []
...
def _allow_live_openclaw_ops(root: Path) -> bool:
    try:
        return root.expanduser().resolve() == CANONICAL_RUNTIME_WORKSPACE
    except Exception:
        return False
...
def _invoke_secondary_codex_worker(
    ...
    if not codex_bin:
        return 5, "", "secondary_codex_missing"
    model = str(model_override or current_model).strip() or str(current_model or "").strip()
    if not model:
        return 5, "", "secondary_codex_not_configured"
    thinking = str(thinking_override or default_thinking(worker_type) or SECONDARY_CODEX_DEFAULT_THINKING).strip() or SECONDARY_CODEX_DEFAULT_THINKING
    if model.startswith("codex-full/"):
    ...
    with tempfile.TemporaryDirectory(prefix="dynamic-worker-") as td:
        ...
        try:
            ...
            return 124, str(exc.stdout or ""), str(exc.stderr or "") or f"secondary_codex_timeout_after_{timeout_value}s"
...
def _run_worker_fallback_chain(
    ...
    prompt = _worker_prompt(worker_type, owner_task_id, task_kind, message)
    fallback = model_plane_run_secondary_then_qwen_fallback(
        "worker",
        current_model,
        default_thinking(worker_type),
        prompt,
        timeout_seconds,
        worker_id,
        reason=reason,
        source=source,
        invoke_codex_exec=lambda timeout_override_seconds, model_override="", thinking_override="": _invoke_secondary_codex_worker(
            config,
            worker_type,
            owner_task_id,
            task_kind,
    ...
    if fallback is None:
        return 5, "", _compact(f"fallback_unavailable:{reason}", 220), "codex_exec", f"codex_exec:{worker_id}", "degraded"

    rc, stdout, stderr, backend_ref = fallback
    backend_ref_token = str(backend_ref or "").strip().lower()
    backend = "qwen" if backend_ref_token.startswith("qwen:") else "codex_exec"
    route_reason = "qwen_fallback" if backend == "qwen" else "secondary_codex_fallback"
    return rc, stdout, stderr, backend, backend_ref, route_reason
...
def plan_worker(config: WorkerManagerConfig, role: str, worker_type: str, owner_task_id: str, task_kind: str) -> dict[str, Any]:
    ...
    if allowed and str(worker_type or "").strip() not in COMPAT_ALLOWED_WORKER_TYPES:
        allowed = False
        reason = f"legacy_worker_type_disabled:{worker_type}"
    ...
    return {
        ...
        "legacy_worker_count": len(records),
    }
...
def run_worker(
    ...
    record = WorkerRecord(
        ...
        metadata={
            "thinking": thinking or default_thinking(worker_type),
            "backend_route_reason": initial_backend_route_reason,
        },
    )
    ...
    if chosen_backend == "mock":
        ...
        rc, stdout, stderr, chosen_backend, backend_ref, backend_route_reason = _run_worker_fallback_chain(
            ...
            "codex_exec_unavailable" if chosen_backend != "codex_exec" else "compat_worker_exec",
            "compat_worker_exec",
        )
    if backend_route_reason == "none" and initial_backend_route_reason != "none":
        backend_route_reason = initial_backend_route_reason
    ...
    payload["ok"] = rc == 0
    payload["rc"] = rc
    payload["legacy_compat_only"] = True
    payload["compat_only"] = True
    payload["storage_plane"] = "runtime_mutable"
    payload["registry_secondary_only"] = True
    payload["events_secondary_only"] = True
    payload["provider_policy_plane"] = "model_plane"
    payload["operator_plane"] = "legacy_compat"
    payload["backend_route_reason"] = backend_route_reason
    if chosen_backend == "qwen":
...
def collect_worker(config: WorkerManagerConfig, role: str, worker_id: str, owner_task_id: str, mark_merged: bool) -> tuple[int, dict[str, Any]]:
    ...
    payload["events_secondary_only"] = True
    payload["provider_policy_plane"] = "model_plane"
    payload["operator_plane"] = "openclaw"
    if not mergeable:
...
def cleanup_workers(config: WorkerManagerConfig) -> dict[str, Any]:
    ...
    orphan_removed = _cleanup_orphan_agents(config, kept)
    return {
        "ok": True,
        "legacy_compat_only": True,
        "compat_only": True,
        ...
        "remaining": len(kept),
        "legacy_worker_count": len(kept),
        "results_path": str(config.results_dir.relative_to(config.root)),
    }


def status_snapshot(config: WorkerManagerConfig, role: str = "") -> dict[str, Any]:
    runtime_truth = build_runtime_truth_snapshot(config.root, state_limit=16, event_limit=16)
    ...
    if event_store_primary:
        return {
            ...
            "active_storage_root": "secondary_compat_only",
            "events_path": "secondary_compat_only",
            "registry_path": "secondary_compat_only",
            "secondary_registry_path": "secondary_compat_only",
            "results_path": "secondary_compat_only",
        }

    records, removed = _cleanup_records(config, records)
    if removed:
        _save_registry(config.registry_path, records)
    worker_orphans = _worker_orphan_ids(records) if _allow_live_openclaw_ops(config.root) else []
    ...
    return {
        ...
        "max_active": config.max_active,
        "legacy_worker_count": len(filtered),
        "active": active,
        "recent": recent,
        "worker_orphan_count": len(worker_orphans),
        "worker_orphans": worker_orphans[:20],
        "cleanup_recommended": bool(worker_orphans),
        "active_storage_root": _display_path(config.registry_path.parent),
        "compat_registry_present": config.registry_path.exists(),
        "compat_events_present": config.events_path.exists(),
        "compat_results_present": config.results_dir.exists(),
        "events_path": "secondary_compat_only",
        "registry_path": "secondary_compat_only",
        "secondary_registry_path": "secondary_compat_only",
        "results_path": "secondary_compat_only",
...
def prompt_context(config: WorkerManagerConfig, role: str) -> str:
    ...
    return (
        ...
        f"worker_compat_only=1 | "
        f"worker_storage_plane=runtime_mutable | "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_run.add_argument("--backend", default="auto", choices=["auto", "mock"])

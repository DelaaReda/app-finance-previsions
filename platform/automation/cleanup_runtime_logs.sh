...
RUNS_DIR_CANONICAL="${ROOT}/evidence/runtime/orchestrator-runs"
RUNS_DIR="${RUNS_DIR_CANONICAL}"
if [[ ! -d "$RUNS_DIR" ]]; then
  echo "BLOCKER: required directory missing: $RUNS_DIR"
  exit 10
fi

RUNS_ARCHIVE_ROOT_CANONICAL="${ROOT}/evidence/runtime/orchestrator-runs-archive"
RUNS_ARCHIVE_ROOT="${RUNS_ARCHIVE_ROOT_CANONICAL}"
mkdir -p "$RUNS_ARCHIVE_ROOT"

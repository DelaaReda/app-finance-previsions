...
def peer_contracts_hint(state_dir: Path, role: str) -> str:
    ...
    if not hints:
        return "none"
    return compact_text("; ".join(hints), 600)
...
def main() -> int:
    ...
    workstate_hint = compact_file_tail(root / "docs/planning/WORKSTATE.md", 20, 320)

    parallel_script = root / "scripts/parallel_workstream.py"
    parallel_hint = run_parallel_workstream(parallel_script, role, "status", 3, 380)
    workboard_context = run_parallel_workstream(parallel_script, role, "context", 3, 520)
    publication_channels = run_parallel_workstream(parallel_script, role, "channels", 4, 620)

    agent_memory = compact_file_tail(role_memory_dir / f"{role}.md", 24, 420)
    self_last_contract = compact_text(read_last_contract_hint(last_contract_file, "self"), 420)

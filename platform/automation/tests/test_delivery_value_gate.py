...
class DeliveryValueGateTests(unittest.TestCase):
    ...
    def test_planner_doc_only_normalizes_path_style_architecture_and_verify(self) -> None:
        payload = "\n".join([
            "STATUS: IN_PROGRESS",
            "DELTA: DONE",
            "EVIDENCE: task_update=complete; lock_check=ok; run_note=planner closure with split compound fields; planner_artifact=docs/operations/orchestrator/proofs/BATCH-58-ANALYSIS.md; root_cause=dependency funnel; fix_applied=close analysis then claim next ready; verify=before=quality_fields_missing; test=python3 platform/automation/compat/projections/parallel_workstream.py context --role planner --limit 5; tests_run=NONE(planner_doc_only); cmd=SKIP(planner_doc_only); stream_id=BATCH-58; architecture_check=docs/architecture/ARCHITECTURE_MAP.md; architecture_plan_ref=docs/architecture/ARCHITECTURE_MAP.md; vision_alignment=batch=BATCH-unknown; target=data_ingestion_freshness_slo; impact=artefact_arch_pour_debloquer_DEV_01; issues=none; issue_count=0; issue_severity=none",
            "RISKS: none",
            "NEXT: owner=planner; action=done",
            "VERDICT: GO_WITH_CAUTION",
            "BLOCKER_ID: NONE",
    ...
    def test_planner_doc_only_detects_none_marker(self) -> None:
        payload = "\n".join([
            "STATUS: IN_PROGRESS",
            "DELTA: DONE",
            "EVIDENCE: task_update=complete; lock_check=ok; run_note=planner closure with NONE doc-only marker; planner_artifact=docs/operations/orchestrator/proofs/BATCH-58-ANALYSIS.md; root_cause=dependency funnel; fix_applied=close analysis then claim next ready; verify=before=quality_fields_missing; test=python3 platform/automation/compat/projections/parallel_workstream.py context --role planner --limit 5; tests_run=NONE(planner_doc_only); commit_sha=NONE(doc_only); stream_id=BATCH-58; architecture_check=docs/architecture/ARCHITECTURE_MAP.md; architecture_plan_ref=docs/architecture/ARCHITECTURE_MAP.md; vision_alignment=batch=BATCH-unknown; target=data_ingestion_freshness_slo; impact=artefact_arch_pour_debloquer_DEV_01; issues=none; issue_count=0; issue_severity=none",
            "RISKS: none",
            "NEXT: owner=planner; action=done",
            "VERDICT: GO_WITH_CAUTION",
            "BLOCKER_ID: NONE",

...
EXEC_LATEST="${DG_ROUTER_EXEC_LATEST_FILE:-$ROOT/logs-codex-runs/orchestrator-state/executors-monitoring-latest.json}"
...
FORCE_RUN_CMD_TIMEOUT_SECONDS="${DG_ROUTER_FORCE_RUN_CMD_TIMEOUT_SECONDS:-25}"
FORCE_RUN_CRON_TIMEOUT_MS="${DG_ROUTER_FORCE_RUN_CRON_TIMEOUT_MS:-20000}"
...
maybe_force_run_admin_agents() {
  ...
  # Keep router non-blocking: cap force-run call duration to avoid cron self-timeout.
  local rc=0
...
PY
    ...
    set +e
    openclaw cron run --timeout "$FORCE_RUN_CRON_TIMEOUT_MS" "$cron_id" >/dev/null 2>&1
    rc=$?
    set -e
  fi

...
def _build_prompt(target_role: str, owner_task_id: str, task_kind: str, message: str) -> str:
    ...
    if target_role in {"dev", "app-dev"}:
        return common + (
            ...
            "- In API wave mode (`BATCH-900`), you may ship one whole endpoint end-to-end: shared contract if needed, application/* logic, ..._endpoint_service.py, thin route, targeted tests, and public EC2 smoke.\n"
            ...
            "- Do not re-invent a decision engine outside reusable services, and do not split the wave back into DEV-01/02/03 micro-fixes.\n"
    ...
    if target_role == "admin":
        return common + (
            ...
            "- In API wave mode, do not wake admin for provider/dependency/business blockers; those defer or route scrum_master instead.\n"
            "- If it is not a runtime/control-plane/public-proof issue, set recommended_next=planner_route_to_dev_or_scrum and say why.\n"
        )
    return common + (
        "Scrum role:\n"
        ...
        "- In API wave mode, you are invoked only after two non-runtime blocks on the same endpoint; unblock/defer coordination only, no admin re-open or ANALYSIS loop.\n"

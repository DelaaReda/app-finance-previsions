...
REQUIRED_REFLECTION_DIMENSIONS = ("scope", "dependency_impact", "risk", "verification", "rollback")
REFLECTION_DIMENSION_ALIASES = {
    "scope": "scope",
    "perimeter": "scope",
    "perimetre": "scope",
    "dependency": "dependency_impact",
    "dependencies": "dependency_impact",
    "dependance": "dependency_impact",
    "dependances": "dependency_impact",
...
def _parse_streak_value(raw: str) -> int:
    value = (raw or "").strip()
    if not value:
        return 0
...
def _delivery_probe_threshold() -> int:
    threshold = _safe_int(os.environ.get("TMUX_ROLE_DELIVERY_PROBE_THRESHOLD", "3"), 3)
    if threshold < 2:
        return 2
    return threshold


def _previous_delivery_probe_streak(role: str, current_task_id: str) -> int:
    state_dir = Path(os.environ.get("TMUX_ROLE_STATE_DIR", "/home/venom/.openclaw/cron/role-state"))
    last_contract_path = state_dir / f"{role}.last_contract"
    if not last_contract_path.exists():
        return 0
    try:
        prev_text = last_contract_path.read_text(encoding="utf-8", errors="ignore")
    except Exception:
        return 0
    prev_values = _parse_contract(prev_text)
    prev_delta = str(prev_values.get("DELTA", "")).strip().upper()
    if prev_delta != "DELIVERY_PROBE_INCONSISTENT_CONTINUE":
        return 0
    prev_evidence = _parse_evidence_kv(prev_values.get("EVIDENCE", ""))
    prev_task_id = (prev_evidence.get("task_id") or "none").strip() or "none"
    current_task = (current_task_id or "none").strip() or "none"
    if prev_task_id != current_task:
        return 0
    explicit = _parse_streak_value(prev_evidence.get("delivery_probe_streak", ""))
    if explicit > 0:
        return explicit
    return 1
...
def _normalize_reflection_dimension(token: str) -> str:
    normalized = re.sub(r"[^a-z0-9_ -]+", "", (token or "").strip().lower())
    normalized = normalized.replace("-", "_").replace(" ", "_")
    return REFLECTION_DIMENSION_ALIASES.get(normalized, normalized)


def _parse_reflection_dimensions(raw: str) -> set[str]:
    dimensions: set[str] = set()
    for fragment in re.split(r"[,/|]+", raw or ""):
        normalized = _normalize_reflection_dimension(fragment)
        if normalized:
...
def main() -> int:
    ...
    def emit_permission_continue(reason: str, writable_refs: list[str] | None = None) -> None:
        ...
        threshold = _delivery_probe_threshold()
        prev_streak = _previous_delivery_probe_streak(role, task_id)
        streak = prev_streak + 1
        run_note = "faux signal lock ignore, reprise livraison exigee"
        if streak >= threshold:
            evidence = (
                "task_update=blocked; lock_check=ok; "
                "run_note=boucle delivery probe detectee sans progression concrete; "
                "exec_report=delivery_probe_streak_exceeded; "
                "issues=delivery_probe_streak_exceeded; "
                "suggestions=executer_claim_ou_complete_manuel_et_revoir_prompt_role; "
                f"stream_id={stream_id}; task_id={task_id}; "
                "tool_request=none; skill_request=none; "
                "channels_read=runtime_context; impact_assessment=high; impact_action=escalate_admin_and_reassign; "
                f"arch_rule=api_contract; review_scope={role}_delivery_probe; conformance=BLOCKED; violations=delivery_probe_streak_exceeded; "
                f"{required_artifact_key}=delivery_probe_streak_exceeded; "
                f"queue_version={runtime_queue_version}; workboard_version={runtime_workboard_version}; "
                f"coordination_ref=delivery_probe_streak:{task_id}; probe_reason={reason_clean}; "
            ...
            values["RISKS"] = "boucle de livraison sans progression concrete"
            values["NEXT"] = (
                "owner=adminapp-codex; action=force_recovery_role_et_reassigner_task_update_concret"
            )
            values["VERDICT"] = "BLOCKED"
            values["BLOCKER_ID"] = "DELIVERY_PROBE_STREAK_EXCEEDED"
            values["NEXT_ACTION_UNIQUE"] = f"ESCALATE_DELIVERY_PROBE_{role.upper()}_{now}"
    ...
    if allow_file_edits and task_update in {"claim", "complete", "handoff"}:
        ...
        reflection_passes_raw = evidence_kv.get("reflection_passes", "").strip()
        reflection_passes = _safe_int(reflection_passes_raw, 0)
        if reflection_passes < 5:
            emit_blocked(
                "REFLECTION_PASSES_INVALID",
                f"role={role}; source={source}; task_update={task_update}; reflection_passes={reflection_passes_raw or 'missing'}; required>=5",
            )
        reflection_dimensions_raw = evidence_kv.get("reflection_dimensions", "").strip()
        if not reflection_dimensions_raw:
            emit_blocked(
                "REFLECTION_DIMENSIONS_MISSING",
                f"role={role}; source={source}; task_update={task_update}; required=reflection_dimensions with {','.join(REQUIRED_REFLECTION_DIMENSIONS)}",
            )
        reflection_dimensions = _parse_reflection_dimensions(reflection_dimensions_raw)
        missing_reflection_dimensions = [
            dim for dim in REQUIRED_REFLECTION_DIMENSIONS if dim not in reflection_dimensions
        ]
        if missing_reflection_dimensions:
            emit_blocked(
                "REFLECTION_DIMENSIONS_INCOMPLETE",
                f"role={role}; source={source}; task_update={task_update}; missing={','.join(missing_reflection_dimensions)}; reflection_dimensions={reflection_dimensions_raw}",
            )
    ...
    evidence_raw = values.get("EVIDENCE", "").upper()
    is_fallback_checkpoint = source == "fallback_checkpoint" and "FALLBACK_MODE=CHECKPOINT" in evidence_raw
    if required_evidence_keys and not is_fallback_checkpoint:
    ...
    if role in {"planner", "dev", "backend_engineer", "frontend_engineer", "integrator", "data_analyst", "infra_engineer", "tester", "qa"} and allow_file_edits and task_update == "blocked" and permission_claimed:
        cmd_value = evidence_kv.get("cmd", "").strip()
        ...
        probe_sources_l = probe_sources.lower()
        lock_markers = (
            "openclaw-shared-locks",
        ...
        mentions_lock = any(marker in probe_sources_l for marker in lock_markers)
        mentions_business_target = any(marker in probe_sources_l for marker in business_markers)
        if mentions_lock and not mentions_business_target:
            emit_permission_continue(
                f"role={role}; source={source}; lock_only_permission_signal",
            )
        ...
        known_probe_paths = {
            ...
            ".tmp/openclaw-shared-locks",
            "/home/venom/analyse-financiere/.tmp/openclaw-shared-locks",
            "/home/venom/shared/analyse-financiere/.tmp/openclaw-shared-locks",
    ...
    if is_fallback_checkpoint and has_artifact_marker:
        print(_render_contract(values))
        return 0

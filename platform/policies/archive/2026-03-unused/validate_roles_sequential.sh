...
ready_batch_ids_csv() {
  ...
  jq -r '[.items[]? | select((.state // "")=="READY") | (.id // "")] | map(select(length>0)) | join(",")' \
    logs-codex-runs/orchestrator-state/priority-queue.json 2>/dev/null || echo ""
}

batch_target_is_ready() {
  local target="$1"
  if [[ -z "$target" || ! -f "logs-codex-runs/orchestrator-state/priority-queue.json" ]]; then
    return 1
  fi
  jq -e --arg id "$target" '.items[]? | select(.id==$id and (.state // "")=="READY")' \
    logs-codex-runs/orchestrator-state/priority-queue.json >/dev/null 2>&1
}
...
resolve_role_map() {
  ...
  for i in "${!ROLE_MAP[@]}"; do
    role="${ROLE_MAP[$i]%%:*}"
    id="$(printf '%s\n' "$cron_json" | jq -r --arg role "$role" '.jobs[] | select(.agentId==$role or .name==($role + "-tmux-loop") or .name==($role + "-tmux") or .name==($role + "-tmux-role") | .id' | head -n 1)"
    ROLE_MAP[$i]="${role}:${id}"
  done
}

apply_role_filter_ids() {
  if [[ "${#ROLE_FILTER_MAP[@]}" -eq 0 ]]; then
    return
  fi
  ...
  for pair in "${ROLE_MAP[@]}"; do
    role="${pair%%:*}"
    id="${pair#*:}"
    if [[ -n "${ROLE_FILTER_MAP[$role]:-}" ]] && [[ -n "$id" && "$id" != "null" ]]; then
      filtered+=("$pair")
    fi
  done
  ROLE_MAP=("${filtered[@]}")

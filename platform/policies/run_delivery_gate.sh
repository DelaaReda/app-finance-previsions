...
RUNS_DIR_CANONICAL="${WORKDIR}/evidence/runtime/orchestrator-runs"
RUNS_DIR="${RUNS_DIR_CANONICAL}"
if [[ ! -d "$RUNS_DIR" ]]; then
  echo "BLOCKED: missing runs dir: $RUNS_DIR"
  exit 16
fi
...
if [[ -z "$ARTIFACT_PATH" ]]; then
  ...
  echo "Example: scripts/run_delivery_gate.sh evidence/gates/openclaw-gates/batch-01-20260224-2200.md"
  exit 2
fi
...
if ! rg -n "(CodexReviewer|codex review --uncommitted|independent_review_gate)" "$ARTIFACT_PATH" -i >/dev/null 2>&1; then
  LATEST_TRANSCRIPT="$(ls -1dt "$RUNS_DIR"/*/transcript.md 2>/dev/null | head -n 1 || true)"
...
if [[ "$review_ref_ok" -ne 1 || "$review_verdict_ok" -ne 1 ]]; then
  LATEST_TRANSCRIPT="${LATEST_TRANSCRIPT:-$(ls -1dt "$RUNS_DIR"/*/transcript.md 2>/dev/null | head -n 1 || true)}"

...
def get_dashboard_kpis() -> Dict[str, Any]:
    ...
    try:
        ...
        kpis_data = load_json("dashboard/kpis")
        
        if kpis_data:
            # Si c'est un wrapper avec metadata, extraire les données
            if "data" in kpis_data:
                return kpis_data["data"]
            elif "payload" in kpis_data:
                return kpis_data["payload"]
            else:
                return kpis_data

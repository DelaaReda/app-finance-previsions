...
SECURITY_FILE="$REPORTS_DIR/security_findings.txt"
{
    ...
    credential_hits=$(find "$BACKEND_DIR" -name "*.py" -exec grep -Hni -E "(secret|token|password|api_key|key.*=)" {} \; 2>/dev/null | grep -vi "demo\|test\|stub\|placeholder\|_test\|test_" | wc -l)
...
STYLE_FILE="$REPORTS_DIR/style_findings.txt"
{
    ...
    debt_markers=$(grep -r -n -E "(TODO|FIXME|HACK|\#.*todo|\#.*fixme)" "$BACKEND_DIR" --include="*.py" | wc -l)
    echo "Technical Debt:"
...
PERFORMANCE_FILE="$REPORTS_DIR/performance_findings.txt"
{
    ...
    perf_hits=$(grep -r -n -E "(for.*in.*range\(.*[0-9]{4}|\bsleep\b|time\.sleep)" "$BACKEND_DIR" --include="*.py" | wc -l)
    echo "Performance Patterns:"
    echo "  - Potentially inefficient loops/sleeps: $perf_hits"
    if [ "$perf_hits" -gt 0 ]; then
        echo "  - Details (first 10):"
        grep -r -n -E "(for.*in.*range\(.*[0-9]{4}|\bsleep\b|time\.sleep)" "$BACKEND_DIR" --include="*.py" | head -10
    fi
    echo ""
...
ERROR_FILE="$REPORTS_DIR/error_handling_findings.txt"
{
    ...
    bare_excepts=$(grep -r -c "except:" "$BACKEND_DIR" --include="*.py" | grep -v -E "(Exception|ValueError|TypeError|IOError|FileNotFoundError)" | awk -F: '{sum+=$2} END {print sum+0}' || echo 0)
    ...
    if [ "$bare_excepts" -gt 0 ]; then
        ...
        grep -r -n "except:" "$BACKEND_DIR" --include="*.py" | grep -v -E "(Exception|ValueError|TypeError|IOError|FileNotFoundError)" | head -5
    fi
    echo ""

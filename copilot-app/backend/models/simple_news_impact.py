"""
Simple News Impact Model for FC-API-030
...
class SimpleNewsImpactModel:
        ...
        try:
            ...
            for news_item in relevant_news:
                try:
                    ...
                    price_before = self._find_closest_price(ticker_prices, news_dt, offset_hours=-24)  # 24h before
                    price_after = self._find_closest_price(ticker_prices, news_dt, offset_hours=48)   # 48h after
    ...
    def calculate_impact_score(self, 
        ...
        try:
            ...
            correlation_impact = abs(price_correlation) * 0.4
            reliability_impact = news_reliability * 0.15
            time_impact = time_weight * 0.05
            
            impact = min(1.0, sentiment_impact + correlation_impact + reliability_impact + time_impact)
    ...
    def analyze_news_impact(self, 
        ...
        """
        Analyze comprehensive news impact on assets
        """
        try:
            result = {
            ...
            if target_tickers:
                ...
                for article in articles:
                    ...
                    # Try extracting from content using regex
                    title = article.get("title", "").upper()
                    description = article.get("description", "").upper()
                    ...
                    valid_tickers = [
                        ...
                        if t not in {'THE', 'AND', 'FOR', 'NOT', 'HAS', 'GET', 'CAN', 'NOW', 'NEW', 
                                   'END', 'SET', 'RUN', 'ALL', 'ANY', 'USD', 'EUR', 'GBP', 'COM', 
                                   'INC', 'LTD', 'CORP', 'DATE', 'TIME', 'USER', 'API'}
                    ]
            ...
            for ticker in tickers_to_analyze:
                if ticker in price_history:
                    ...
                    # Calculate aggregate sentiment for this ticker
                    ticker_sentiments = []
                    for article in articles:
                        title = article.get("title", "").upper()
                        description = article.get("description", "").upper()
                    ...
                    impact_score = self.calculate_impact_score(
                        avg_sentiment,
                        correlation,
                        0.6,  # Default reliability
                        1.0   # Default time impact
                    )

  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (data && data.length > 0) {
      const parsedData = data.map(d => ({
        date: new Date(d.date),
        value: parseFloat(String(d.value)),
        x: 30 + ((new Date(d.date).getTime() - Math.min(...data.map(d => new Date(d.date).getTime()))) / (Math.max(...data.map(d => new Date(d.date).getTime())) - Math.min(...data.map(d => new Date(d.date).getTime()))) * (width! - 45)
      })).filter(d => !isNaN(d.date.getTime()) && !isNaN(d.value))

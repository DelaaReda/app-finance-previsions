...
try:
    from services.g4f_client import _fastest_tested_model
except Exception:  # pragma: no cover
    _fastest_tested_model = None  # type: ignore
...
def run_llm_summary(save_base: str = "data/llm_summary") -> LLMEnsembleSummary:
    ...
    tested_model = None
    if _fastest_tested_model is not None:
        try:
            tested = _fastest_tested_model()
            if isinstance(tested, tuple) and len(tested) == 2:
                tested_model = tested[1]
        except Exception:
            tested_model = None

    default_summary_model = (
        llm_settings.llm_summary_model
        if llm_settings is not None
        else (tested_model or "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo")
    )
    configured_summary_model = os.getenv("LLM_SUMMARY_MODEL", default_summary_model)
    if not configured_summary_model and llm_settings is not None:
        configured_summary_model = llm_settings.llm_summary_model
    client = LLMClient(provider="g4f", model=configured_summary_model)

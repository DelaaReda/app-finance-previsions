...
Focus uniquement sur logs RECENTS (pas anciens) pour les 7 cron roles.
Objectif du tick:
...
deterministic_delivery_tick() {
  ...
  case "$top_issue" in
    ...
    roles_disabled_admins_only_mode)
      ...
      ;;
    stale_running_jobs|sessions_missing|role_errors_present|role_jobs_pending|role_jobs_missing|role_jobs_disabled|sessions_stale_no_recent_runner_activity)
